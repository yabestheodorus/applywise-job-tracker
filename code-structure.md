...
- **Authenticated shell** (`app/(app)/`): a route-group `layout.tsx` renders a left **sidebar** (`components/app-sidebar.tsx`: Board / Upcoming / Profile / Stages / Templates, active-link aware) + the top `AppHeader` (Add-application dialog, theme toggle, user menu) + `Toaster`. The board is `app/(app)/page.tsx`; Stages/Templates are placeholder pages for now. (The old top-bar-only `app/page.tsx` and `lib/mock.ts` were removed.)
