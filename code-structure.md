...
- **`proxy.ts`** (Next 16 "proxy" convention, formerly `middleware.ts`): on every request, refreshes the session and **gates access** — unauthenticated users → `/login`, signed-in users are bounced off the landing/`/login`/`/signup` pages straight to `/board`. Public paths: `/` (the marketing landing page), `/login`, `/signup`, `/auth`.
...
- **Public landing page** (`app/page.tsx`): a marketing page at `/` (no app shell, root layout only) — sticky nav with Log in / Get started, hero with a decorative board preview, problem framing, a 6-feature grid, an Interview-Training-Session spotlight, a 3-step "how it works", a spreadsheet differentiator, and CTAs to `/signup`. Public (allow-listed in the proxy); signed-in visitors are redirected to `/board`.
